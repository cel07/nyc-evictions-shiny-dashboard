...
# App directory (where index.qmd lives in deployment)
APP_DIR = Path(os.environ.get("QUARTO_PROJECT_DIR", Path.cwd())).resolve()

# Data directories MUST be inside the deployed app folder
...
marshal_map_global = {name: f"Marshal {i+1}" for i, name in enumerate(all_unique_marshals)}

evictions_clean["marshal_id"] = (
    evictions_clean["marshal_full_name"].map(marshal_map_global).fillna("Unknown Marshal")
)
